...
class PlaceDB (object):
    ...
    def density_map(self, x, y):
        ...
        bin_index_xh = np.minimum(np.ceil((x+self.node_size_x[: self.num_physical_nodes])/self.bin_size_x).astype(np.int32), self.num_bins_x-1)
        bin_index_yl = np.maximum(np.floor(y/self.bin_size_y).astype(np.int32), 0)
        bin_index_yh = np.minimum(np.ceil((y+self.node_size_y[: self.num_physical_nodes])/self.bin_size_y).astype(np.int32), self.num_bins_y-1)
    ...
    def update_density_map(self, x, y, node):
        index = self.node_name2id_map[node]
        bin_index_xl = np.maximum(np.floor(x/self.bin_size_x).astype(np.int32), 0)
        bin_index_xh = np.minimum(np.ceil((x+self.node_size_x[index])/self.bin_size_x).astype(np.int32), self.num_bins_x-1)
        bin_index_yl = np.maximum(np.floor(y/self.bin_size_y).astype(np.int32), 0)
        bin_index_yh = np.minimum(np.ceil((y+self.node_size_y[index])/self.bin_size_y).astype(np.int32), self.num_bins_y-1)
        for ix in range(bin_index_xl, bin_index_xh+1):
            for iy in range(bin_index_yl, bin_index_yh+1):
                self.density[ix, iy] += self.overlap(
                            self.bin_xl(ix), self.bin_yl(iy), self.bin_xh(ix), self.bin_yh(iy),
                            x, y, x+self.node_size_x[index], y+self.node_size_y[index]
                            )

    def current_density_map(self):
        ans = self.density.copy()
        for ix in range(self.num_bins_x):
            for iy in range(self.num_bins_y):
                ans[ix, iy] /= (self.bin_xh(ix)-self.bin_xl(ix))*(self.bin_yh(iy)-self.bin_yl(iy))

        return ans

    def density_overflow(self, target_density):
        """
...
""" % (
        ...
        self.density = np.zeros([self.num_bins_x, self.num_bins_y])
    ...
    def write_pl_free(self, pl_file):
        content = "UCLA pl 1.0\n"
        str_node_names = np.array(self.node_names).astype(np.str)
        for i in range(self.num_physical_nodes):
            content += "\n%s\t%g\t%g\t: %s" % (str_node_names[i], 0, 0, 'N')
        content += '\n'
        with open(pl_file, "w") as f:
            f.write(content)

    def write_nodes_free(self, nodes_file):
        content = "UCLA nodes 1.0\n"
        content += "\nNumNodes : \t\t" + str(self.num_physical_nodes)
        content += "\nNumTerminals : \t\t0"
        str_node_names = np.array(self.node_names).astype(np.str)
        for i in range(self.num_physical_nodes):
            content += "\n\t%s\t%g\t%g" % (str_node_names[i], self.node_size_x[i], self.node_size_y[i])
        content += '\n'
        with open(nodes_file, "w") as f:
            f.write(content)

    def write_pl_placed(self, pl_file, macro_names, manager):
        content = "UCLA pl 1.0\n"
        str_node_names = np.array(self.node_names).astype(np.str)
        for i in range(self.num_physical_nodes):
            if str_node_names[i] in macro_names:
                node_x = manager.items[manager.name2id[str_node_names[i]]].place_x + self.xl
                node_y = manager.items[manager.name2id[str_node_names[i]]].place_y + self.yl
                content += "\n%s\t%g\t%g\t: %s /FIXED" % (str_node_names[i], node_x, node_y, 'N')
            else:
                content += "\n%s\t%g\t%g\t: %s" % (str_node_names[i], self.node_x[i], self.node_y[i], 'N')
        content += '\n'
        with open(pl_file, "w") as f:
            f.write(content)

    def write_def_placed(self, chip, def_file, macro_index, xs, ys):
        content = ''
        str_node_names = np.array(self.node_names).astype(np.str)
        for index in macro_index:
            str_node_names[index] = str_node_names[index].split('.')[0]
        macroNames = str_node_names[macro_index]
        fr = open('./benchmarks/iccad2015/' + chip + '/' + chip + '_macro_isolate.def', 'r')
        flag = False
        name = None
        for line in fr.readlines():
            if not flag:
                lineSplit = line.strip().split()
                if len(lineSplit) > 0 and lineSplit[0] == 'PINS':
                    flag = True
                if len(lineSplit) > 2 and lineSplit[0] == '-':
                    name = str(lineSplit[1])
                if 'FIXED' in lineSplit and name in macroNames:
                    index = np.where(macroNames == name)[0][0]
                    lineSplit[3] = str(xs[index])
                    lineSplit[4] = str(ys[index])
                    line = '  '
                ...
                content += line
        fr.close()
        fr = open('./benchmarks/iccad2015/' + chip + '/' + chip + '_' + def_file + '.def', 'w')
        fr.write(content)
        fr.close()

    def write_pl_placed_position(self, pl_file, macro_index, xs, ys):
        content = "UCLA pl 1.0\n"
        str_node_names = np.array(self.node_names).astype(np.str)
        for i in range(self.num_physical_nodes):
            if i in macro_index:
                index = np.where(macro_index == i)[0][0]
                content += "\n%s\t%g\t%g\t: %s /FIXED" % (str_node_names[i], xs[index] + self.xl, ys[index] + self.yl, 'N')
            else:
                content += "\n%s\t%g\t%g\t: %s" % (str_node_names[i], self.node_x[i], self.node_y[i], 'N')
        content += '\n'
        with open(pl_file, "w") as f:
    ...
    def write_pl_placed_position_sep(self, pl_file, chip, macroNames, xs, ys):
        content = ''
        macroNames = np.array(macroNames)
        fr = open('./benchmarks/dac2012/' + chip + '/' + chip + '_macro.pl', 'r')
        for line in fr.readlines():
            lineSplit = line.strip().split()
            if len(lineSplit) > 0 and lineSplit[0] in macroNames:
                index = np.where(macroNames == lineSplit[0])[0][0]
                line = '\t' + lineSplit[0] + '\t' + str(int(xs[index] + self.xl)) + '\t' + str(int(ys[index] + self.yl)) + '\t' + lineSplit[3] + '\t' + lineSplit[4] + '\t' + lineSplit[5] + '\n'
            content += line
        fr.close()
        with open(pl_file, "w") as fr:
            fr.write(content)
